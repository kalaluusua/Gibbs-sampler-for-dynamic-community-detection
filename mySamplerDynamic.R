...
SBM_new <- function(data, data1, niterations, beta.a, beta.b, GAMMA, LAMBDA, nClusters, nLayers = 1)
{
  ...
  # sample pi from Dirichlet distribution indirectly
  pi <- matrix(0, nClusters)
  for (i in 1:nClusters){
    pi[i] = rgamma(1,gamma)
  }
  pi <- pi/sum(pi)
  ...
  for (iter in 1:niterations)
  {
    ...
    pi <- matrix(0, nClusters)
    for (i in 1:nClusters){
      pi[i] = rgamma(1,sum(clusterAssign==i) + gamma)
    }
    pi <- pi/sum(pi)
    
    History[[iter]] <- list(zout = clusterAssign,Qout = Q,piout = pi)
    # cat(" iteration:", iter,"\n",clusterAssign,"\n")
...
getDahl <- function(data1, fit, burn, nClusters, nLayers = 1)
{
  ...
  ## get pi ##
  for (i in 1:nClusters){
    DahlAns$piout[i] = sum(DahlAns$zout==i)/n
  }
  DahlAns
}
...
NN <- 10 # repeat the simulation study for NN identically distributed networks
errors <- c()
