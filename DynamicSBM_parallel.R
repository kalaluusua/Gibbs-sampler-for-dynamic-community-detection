...
iterate <- 
  function(n, NC, p, q, NL, niterations, burn) {
    theta <- matrix(q,NC,NC) 
    diag(theta) = p 
    
    ## data generation
    Z <- c(sample(1:NC, size = NC, replace = FALSE),
           sample(1:NC, size = n-NC, replace = TRUE,prob = c(1,1))) ## clustering configuration
    Z = Z[order(Z)]
    ...
    # set.seed(1)
    fit1 = SBM_new(data = AT, data1 = AAAT, niterations = niterations, beta.a = 1, beta.b = 1, GAMMA=100*NC, LAMBDA = 1, nClusters = NC, nLayers = NL)
    ...
    ## estimated configuration using Dahl's method, choosing first "burn" iterations in MCMC as burn-in
    result1 = getDahl(data1 = AAAT, fit1, burn = burn, nClusters = NC, nLayers = NL)
    ## result1[[1]] denotes the estimated clustering configuration.
    ## result1[[2]] denotes the estimated Q matrix.
    error <- min(length(which(result1[[1]] != Z)), length(which(result1[[1]] != abs(Z - 3))))/n
    adj_rand_index <- adj.rand.index(result1[[1]], Z)
    ## cat("epoch:", s, "error:", errors[s], "rand (adj):", adj_rand_indices[s],"\n")
    result <- c(error, adj_rand_index)
    return(result)
  }

NN <- 100 # repeat the simulation study for NN >= 2 identically distributed networks
cases <- list(c(0.15, 0.1)) ## c(c(p1,q1), c(p2,q2), ...), where
## p is diagonal value for Q matrix, and
## q is off-diagonal value for Q matrix
n_values <- c(100) ## number of observations
NL_values <- c(1, 3, 5, 7) # Number of layers
...
results <- foreach(i=1:NN, .combine=cbind) %dopar% {
  ###### one example in simulation study
  return(iterate(n = n, NC = NC, p = p, q = q, NL = NL, niterations = niterations, burn = burn))
}
# stop cluster
stopCluster(cl)
end_time <- Sys.time()
...
colnames(results_df) <- c("CE", "AR")

filename <- gsub("\\.", "", paste("N", n, "_", "T", NL, "_", "p", p, "_", "q", q,  sep="")) # remove dots
folder <- paste("./Results-", Sys.Date(), sep="")
filepath <- paste(folder, "/", filename, ".csv", sep="")
dir.create(file.path(".", folder), showWarnings = FALSE)
write.csv(results_df, file = filepath, row.names = FALSE)
